...
echo "$($PSQL "(SELECT w.name FROM teams as w LEFT JOIN games ON w.team_id=games.winner_id WHERE year='2014' AND round='Eighth-Final' UNION SELECT o.name FROM teams as o JOIN games ON games.opponent_id=o.team_id WHERE year='2014' AND round='Eighth-Final') ORDER BY name;")"
